...
def test_oembed_head_meta_title(client):
    respx.get("https://api.fxtwitter.com/fallback/status/999").mock(return_value=Response(500))
    respx.get("https://publish.twitter.com/oembed").mock(return_value=Response(200, json=OEMBED_RESPONSE))
...
def test_both_strategies_fail_falls_through_to_providers(client):
    respx.get("https://api.fxtwitter.com/nobody/status/000").mock(return_value=Response(404))
    respx.get("https://publish.twitter.com/oembed").mock(return_value=Response(404))
...
def test_twitter_com_domain(client):
    respx.get("https://api.fxtwitter.com/user/status/111").mock(return_value=Response(200, json=FXTWITTER_RESPONSE))
...
def test_www_x_com_domain(client):
    respx.get("https://api.fxtwitter.com/user/status/222").mock(return_value=Response(200, json=FXTWITTER_RESPONSE))
...
def test_fxtwitter_thread_walk_stops_at_cycle(client):
    ...
    respx.get("https://api.fxtwitter.com/a/status/100").mock(return_value=Response(200, json=leaf))
    parent_route = respx.get("https://api.fxtwitter.com/_/status/200").mock(return_value=Response(200, json=parent))
    # The loop-back fetch for the leaf must NOT happen — the walker sees id 100
    # in the seen-set and stops.
    loop_route = respx.get("https://api.fxtwitter.com/_/status/100").mock(return_value=Response(200, json=leaf))
...
def test_syndication_fallback_when_fxtwitter_and_oembed_fail(client):
    # fxtwitter down
    respx.get("https://api.fxtwitter.com/user/status/777").mock(return_value=Response(500, text="boom"))
    # oEmbed down
    respx.get("https://publish.twitter.com/oembed").mock(return_value=Response(500, text="boom"))
...
def test_twitter_source_is_oembed_when_fxtwitter_fails(client):
    respx.get("https://api.fxtwitter.com/user/status/888").mock(return_value=Response(500, text="boom"))
    respx.get("https://publish.twitter.com/oembed").mock(return_value=Response(200, json=OEMBED_RESPONSE))
