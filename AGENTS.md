...
FastAPI service that fetches full HTML content from URLs through a stealth-browser provider chain. Designed for AI agents and pipelines that need rendered DOM (not just raw HTTP) without each caller re-implementing anti-bot evasion.

## Stack

- Python 3.12
- FastAPI
- Obscura (Rust headless browser, V8 JS engine, `--features stealth`)
- Scrapling + Camoufox (stealth Firefox)
- Cloudflare Browser Rendering (paid)
- Firecrawl (paid)
...
### `GET /api/v1/agent` and `GET /api/AGENTS.md`

Both return the full content of this `AGENTS.md` file as plain text. Useful for AI agents that consume the scrapi API and need to understand its capabilities, response formats, and error handling at runtime. No auth — intentionally discoverable.

`/api/AGENTS.md` is the canonical, easy-to-remember path; `/api/v1/agent` is kept as an alias for backwards compatibility.
...
(single static file, ~60 MB), or build from source with
`cargo build --release --features stealth` to enable per-session
fingerprint randomization and TLS-fingerprint mimicking. Stealth is
enabled per-fetch via `--stealth`.
...
```bash
curl -H "Authorization: Bearer <token>" "http://localhost:10700/api/v1/content?urls=https://example.com"

curl -H "Authorization: Bearer <token>" "http://localhost:10700/api/v1/content?urls=https://www.nytimes.com/spotlight/lifestyle"

# Opt into a 1h response cache
curl -H "Authorization: Bearer <token>" "http://localhost:10700/api/v1/content?urls=https://example.com&cache=1h"
```

Multiple URLs (repeat the `urls` param):

```bash
curl -H "Authorization: Bearer <token>" "http://localhost:10700/api/v1/content?urls=https://example.com&urls=https://example.org&urls=https://example.net"
```

Asset download (image):

```bash
curl -H "Authorization: Bearer <token>" "http://localhost:10700/api/v1/asset?url=https://www.google.com/images/branding/googlelogo/2x/googlelogo_color_272x92dp.png"
```

Asset download with format conversion and resize:

```bash
curl -H "Authorization: Bearer <token>" "http://localhost:10700/api/v1/asset?url=https://www.google.com/images/branding/googlelogo/2x/googlelogo_color_272x92dp.png&output_format=JPG,95&max_width=800"
```

Asset download (CSS):

```bash
curl -H "Authorization: Bearer <token>" "http://localhost:10700/api/v1/asset?url=https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css"
```

With lazy-load scrolling (for pages that load content on scroll):

```bash
curl -H "Authorization: Bearer <token>" "http://localhost:10700/api/v1/content?urls=https://www.coches.net/segunda-mano/&scroll_full=true"
```
...
- `PROXY_URL` — optional, SOCKS5 proxy URL. Routes scrapling/asset fetches through the proxy. Useful for switching from a datacenter IP to a residential one when the target server blocks the former.
